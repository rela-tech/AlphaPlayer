               ...
               constant BDAlphaPlayerConvertMatrix *convertMatrix [[ buffer(BDAlphaPlayerFragmentInputIndexMatrix) ]])
{
    ...
    float tcx = input.textureCoordinate.x / 2;
    ...
    float3 alpha = float3(textureY.sample(textureSampler, float2(input.textureCoordinate.x / 2 + 0.5, input.textureCoordinate.y)).r,
